...
# Allow local X11 access only when xhost is available.
...
docker run --rm -it -e DISPLAY=${DISPLAY} \
        -v /tmp/.X11-unix:/tmp/.X11-unix:rw --network host --privileged \
        --workdir="/workspace/practica_2" \
        --volume="$SCRIPT_DIR:/workspace/practica_2:rw" -e "TERM=xterm-256color" \
        --name $containerName \
